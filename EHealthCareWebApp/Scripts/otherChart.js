...
window.Chart = function(context, options){
	...
	this.TreeMap = function(data, options) {
		chart.TreeMap.defaults = {
			...
			segmentStrokeColor : "#fff",
			segmentStrokeWidth : 2,
			squareShowLabels : true, // TODO: set to false
			squareLabel : "<%=value%>",
...
var TreeMap = function(data, config, ctx)
	{
		...
		function calculateAndDrawSquares(animPc, nodes, rectX, rectY, rectW, rectH, depth) {
			...
			{
				console.log("\n");
				console.log("nodes count : %d - rect : %f %f %f %f - depth : %d", nodes.length, rectX, rectY, rectW, rectH, depth);
			}
			if(nodes.length <= 1) {
				if(animPc >= 1)
					console.log("draw square color : %s", nodes[0].color);
				ctx.lineWidth = config.segmentStrokeWidth;
				ctx.fillStyle = nodes[0].color;
				ctx.strokeStyle = /*"00FF00"*/config.segmentStrokeColor;
				ctx.beginPath();
			...
			if(animPc >= 1)
				console.log("a count : %d", aArray.length);
			var bArray = nodes.slice(m, nodes.length);
			//var bArray = nodes.slice(m, nodes.length - m);
			if(animPc >= 1)
				console.log("b count : %d", bArray.length);
			...
			if(animPc >= 1)
				console.log("a total : %d", aTotal);
			var bTotal = 0;
			for(var i=0;i<bArray.length;i++) {
				bTotal += bArray[i].value;
			}
			if(animPc >= 1)
				console.log("b total : %d", bTotal);
			...
			if(horizontal) {
				...
				if(animPc >= 1)
					console.log("a rect : %d %d %d %d", aRect["x"], aRect["y"], aRect["width"], aRect["height"]);
				bRect["x"] = rectX + aWidth;
				bRect["y"] = rectY;
				bRect["width"] = bWidth;
				bRect["height"] = bHeight;
				if(animPc >= 1)
					console.log("a rect : %d %d %d %d", bRect["x"], bRect["y"], bRect["width"], bRect["height"]);
			}
			else { // vertical layout
				if (total == 0) {
					if(animPc >= 1)
						console.log("vertical but total 0");
